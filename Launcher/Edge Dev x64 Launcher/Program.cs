...
namespace Edge_Dev_x64_Launcher
{
    static class Program
    {
        ...
        static void Main()
        {
            ...
            if (File.Exists(applicationPath + "\\Edge Dev x64\\msedge.exe"))
            {
                ...
                if (!File.Exists(applicationPath + "\\Edge Dev x64\\Profile.txt"))
                {
                    Application.EnableVisualStyles();
                    Application.SetCompatibleTextRenderingDefault(false);
                    Application.Run(new Form1());
                    String Arguments = File.ReadAllText(applicationPath + "\\Edge Dev x64\\Profile.txt") + sb.ToString();
                    if (Arguments.Contains("--user-data-dir="))
                    {
                        ...
                        _ = Process.Start(applicationPath + "\\Edge Dev x64\\msedge.exe", Arguments3.Replace("'", ""));
                    }
                    else
                    {
                        _ = Process.Start(applicationPath + "\\Edge Dev x64\\msedge.exe", Arguments);
                    }
                }
                else
                {
