...
namespace Edge_Canary_x64_Launcher
{
    public partial class Form1 : Form 
    {
        ...
        private void Button1_Click(object sender, System.EventArgs e)
        {
            ...
            {
                File.WriteAllText(@"Edge Canary x64\Profile.txt", "--user-data-dir=\"profile\"");
                this.Close();
            }
            if (radioButton2.Checked)
            {
                File.WriteAllText(@"Edge Canary x64\Profile.txt", "--user-data-dir=\"Edge Canary x64\\profile\"");
                this.Close();
            }
            if (radioButton3.Checked)
            {
                File.WriteAllText(@"Edge Canary x64\Profile.txt", "");
                this.Close();
            }
        }
    }
